...
ifeq (cleveldb,$(findstring cleveldb,$(MINTSTATION_BUILD_OPTIONS)))
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=mintstation \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=mintstationd \
...
ifeq (,$(findstring nostrip,$(MINTSTATION_BUILD_OPTIONS)))
  ldflags += -w -s
endif
ldflags += $(LDFLAGS)
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/mintstationd

build:
	go build $(BUILD_FLAGS) -o bin/mintstationd ./cmd/mintstationd
